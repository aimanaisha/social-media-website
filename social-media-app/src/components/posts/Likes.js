...
const Likes = (props) => {
    ...
    useEffect(()=>{
        ...
        setUserLike(data.docs.map((doc) => ({...doc.data(), id: doc.id })).filter((like)=>{
            return like.uid === auth.currentUser.uid
        }))
        data.docs.map((doc) => ({...doc.data(), id: doc.id })).map((like)=>{
            if(like.uid === auth.currentUser.uid){setLikeState(true)}
        })
        }
        getLikes()
    }, [likeState])
    ...
    return(
        <>
            <button className={classes.button} onClick={test}>
                <img className={classes.imgbtn} src={likeState === true ? like : unlike1}/>
