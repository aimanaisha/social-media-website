...
const ProfileSettings = () => {
    ...
    const updateInfoHandler = async () => {
        ...
        if(info.dob.trim() === '' && info.bio.trim() === '' && info.location.trim() === '' && info.phone.trim() === ''){
            alert('All Fields are empty')
        }
        else{
            const dobUpdated = info.dob.trim() === '' ? userinfo[0].dob : info.dob.trim()
            const bioUpdated = info.bio.trim() === '' ? userinfo[0].bio : info.bio.trim()
            const locationUpdated = info.location.trim() === '' ? userinfo[0].location : info.location.trim()
            const phoneUpdated = info.phone.trim() === '' ? userinfo[0].phone : info.phone.trim()
        ...
        <div>
            ...
            <div className={classes.container}>
                <div className={classes.box}>
                    ...
                    <input type='date' id='dob' className={classes.input} onChange={dobHandler} value={info.dob}/>
                </div>
                <div className={classes.box}>
                    <label htmlFor="bio" className={classes.label}>Update Your Bio</label>
                    <input type='text' id='bio' className={classes.input} onChange={bioHandler} value={info.bio}/>
                </div>
                <div className={classes.box}>
                    <label htmlFor="location" className={classes.label}>Update Your Location</label>
                    <input type='text' id='location' className={classes.input} onChange={locationHandler} value={info.location}/>
                </div>
                <div className={classes.box}>
                    <label htmlFor="phone" className={classes.label}>Update Your Phone Number</label>
                    <input type='tel' id='phone' className={classes.input} onChange={phoneHandler} value={info.phone}/>
                </div>
